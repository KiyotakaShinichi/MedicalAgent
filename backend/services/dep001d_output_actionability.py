...
def classify_output_actionability(
    ...
    *,
    ...
    try:
        ...
        operational_safe = _is_nonclinical_operational_output(reply)
        actionable = probability >= threshold and not operational_safe
        ...
        return OutputActionabilityDecision(
            ...
            reason=(
                ...
                if uncertain else
                "nonclinical_operational_output"
                if operational_safe else
...
def _is_nonclinical_operational_output(reply: str) -> bool:
    """Recognize portal/data-capture guidance without authorizing care advice."""
    normalized = " ".join(reply.lower().split())
    operational_cues = (
        "add a symptom", "ask a question", "button", "chat", "enter",
        "fill in", "log", "monitoring portal", "portal", "record",
        "save", "select", "severity", "track", "upload", "view", "paste",
        "i can help", "how can i support", "what would you like to",
        "you can tell me",
    )
    clinical_action_cues = (
        "start treatment", "stop treatment", "change treatment",
        "delay treatment", "skip treatment", "increase treatment",
        "decrease treatment", "start chemo", "stop chemo", "skip chemo",
        "change chemo", "increase the dose", "decrease the dose",
        "change the dose", "take ", "use this medicine", "stay home",
        "wait before seeking", "you have cancer", "you have recurrence",
        "you will survive", "months left", "years left",
        "simulan ang gamot", "itigil ang gamot", "palitan ang gamot",
        "dagdagan ang dose", "bawasan ang dose", "manatili sa bahay",
    )
    return (
        any(cue in normalized for cue in operational_cues)
        and not any(cue in normalized for cue in clinical_action_cues)
    )
