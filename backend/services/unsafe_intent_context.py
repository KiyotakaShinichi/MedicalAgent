...
def classify_unsafe_intent_with_context(
    ...
    if combined.get("is_unsafe") and not combined.get("borderline"):
        confidence = max(float(combined.get("confidence") or 0.0), 0.84)
        return {
            **combined,
    ...
    for prior in reversed(prior_messages):
        ...
        if prior_result.get("is_unsafe") and not prior_result.get("borderline"):
            confidence = max(
