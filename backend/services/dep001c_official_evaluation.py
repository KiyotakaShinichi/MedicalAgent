"""One-shot transactional evaluator for immutable DEP-001C evidence."""
...
from backend.services.dep001c_candidate_snapshot import PREDECLARED_GATES, gates_pass
...
def run_dep001c_official_internal_once(
    *,
    candidate_id: str,
    blind_bank_id: str,
    ...
    run_id = f"dep001c-run-{canonical_hash(run_identity)[:20]}"
    run_dir = run_root / run_id
    receipt_path = run_dir / "receipt.json"
    transaction_path = run_dir / "transaction.json"
    if receipt_path.exists() or transaction_path.exists():
        raise IntegrityViolation("official_internal_bank_is_one_shot_and_already_consumed")
    ...
    receipt = {
        "schema_version": "dep001c_official_receipt_v1",
        "run_id": run_id,
        **run_identity,
        "status": "started_no_rerun",
        "started_at": utc_now(),
        "rerun_allowed": False,
    }
    atomic_write_json(receipt_path, receipt)
    transaction: dict[str, Any] = {
        "schema_version": "dep001c_evidence_transaction_v1",
        "run_id": run_id,
        "candidate_id": candidate_id,
        "blind_bank_id": blind_bank_id,
    ...
    try:
        with EvaluationLock(candidate_id=candidate_id, run_id=run_id, lock_root=lock_root) as lock:
            ...
            pre = {
                "schema_version": "dep001c_pre_run_integrity_v1",
            ...
            worker_path = candidate_dir / "source/dep001c_snapshot_worker.py"
            environment = dict(os.environ)
            environment["PYTHONDONTWRITEBYTECODE"] = "1"
            environment["PYTHONPATH"] = str(candidate_dir / "source")
            environment["NLCARE_DEP001B_ARTIFACT_DIR"] = str(candidate_dir / "runtime")
            ...
            if completed.returncode != 0:
                raise IntegrityViolation(f"frozen_worker_failed:{completed.returncode}")
            ...
            checkpoints_passed = bool(
                checkpoints
                and all(row.get("candidate_passed") and row.get("blind_passed") for row in checkpoints)
            )
            post = {
                "schema_version": "dep001c_post_run_integrity_v1",
                "generated_at": utc_now(),
                "candidate": post_candidate,
                "blind_bank": post_blind,
            ...
            post["passed"] = bool(
                post_candidate["passed"]
                and post_blind["passed"]
            ...
            # Behavioral metrics become admissible only after VERIFIED_POST.
            worker = json.loads(worker_output.read_text(encoding="utf-8"))
            candidate_manifest = json.loads(candidate_manifest_path.read_text(encoding="utf-8"))
            assurance_path = candidate_dir / "assurance/runtime_assurance.json"
            assurance = json.loads(assurance_path.read_text(encoding="utf-8"))
            integrity_fault = json.loads(
                (candidate_dir / "assurance/integrity_fault_injection.json").read_text(encoding="utf-8")
            )
            fault = assurance.get("fault_injection") or {}
            fault_rate = (
                float(fault.get("passed_n", 0)) / float(fault.get("total_n", 1))
                if int(fault.get("total_n", 0)) > 0
                else 0.0
            )
            behavior_passed = gates_pass(worker["metrics"], fault_injection_pass_rate=fault_rate)
            integrity_fault_passed = integrity_fault.get("status") == "passed"
            decision = "INTERNAL_PASS_EXTERNAL_REQUIRED" if behavior_passed and integrity_fault_passed else "BLOCKED_BEHAVIORAL"
            result = {
                "schema_version": "dep001c_official_internal_evaluation_v1",
                "generated_at": utc_now(),
                "run_id": run_id,
                "status": decision,
                "integrity_valid": True,
                "behavioral_gates_passed": behavior_passed,
                "candidate_id": candidate_id,
                ...
                "cases_evaluated": worker["case_n"],
                "pre_run_integrity": pre,
                "checkpoint_integrity_passed": checkpoints_passed,
                "post_run_integrity": post,
                "metrics": worker["metrics"],
                "confidence_intervals": worker["confidence_intervals"],
                "failed_case_ids": worker["failed_case_ids"],
                "predeclared_gates": PREDECLARED_GATES,
                "fault_injection": fault,
                "fault_injection_pass_rate": fault_rate,
                ...
                "limitations": [
                    ...
                    "The candidate was not tuned from either burned blind bank.",
                    "A fresh independent no-read external holdout is still required for DEP-001.",
                ],
            }
        ...
        atomic_write_json(run_dir / "invalidated.json", {
            "schema_version": "dep001c_invalidated_run_v1",
            "run_id": run_id,
            "status": "BLOCKED_EVIDENCE_INTEGRITY",
            "behavioral_metrics_admissible": False,
            "failure_type": type(exc).__name__,
            "failure_code": str(exc)[:240],
            "candidate_frozen": False,
            "ready_for_new_external_holdout": False,
            "dep001_status": "BLOCKED",
            "clinical_validation": False,
        })
        raise
...
__all__ = ["run_dep001c_official_internal_once"]
