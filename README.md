...
- Latency: five local routes have 30 fresh observations each. Their current p95 values range from about `46ms` for a validated cache hit to `756ms` for normal RAG; the low-confidence safe-default route is about `719ms`. These are forced-sparse local regression measurements, not production SLO evidence, and `production_ready` remains `false`.
- Stage latency: the refreshed 30-case normal-RAG probe reports about `729ms` total p95 and `648ms` retrieval p95; retrieval is the dominant measured local bottleneck. The legacy 500-row telemetry sample has p95 `3.59s`, p99 `70.76s`, and zero provider-token coverage, so provider billing and production tail behavior remain unproven.
...
- Evidence maturity is reported without an aggregate score: 3 dimensions are scaffold/contract only, 6 have internal self-tests, 2 have frozen internal evidence, and none have independent external or institutional evidence. The architecture budget currently flags 14 oversized files, including 4 critical hotspots.
