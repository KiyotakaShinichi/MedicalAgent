...
- cBioPortal clinical export: `Data/evals/models/latest_cbioportal_clinical_export.json`
- External distribution alignment: `Data/evals/models/latest_external_distribution_alignment.json`
- Common-feature transfer stress: `Data/evals/models/latest_common_feature_transfer_stress.json`
- Public-distribution realism candidate: `Data/evals/models/latest_public_distribution_realism_candidate.json`
- Current-vs-realism candidate A/B gate: `Data/evals/models/latest_realism_candidate_ab_gate.json`
- Dataset expansion deep-search catalog: `Data/evals/models/latest_dataset_expansion_deep_search.json`
...
### Common Public-Bridge Features

The public-data bridge exposes a strict shared feature subset for transfer stress tests:

- `age`
- `baseline_tumor_size_mm` or tumor-size proxy
- `hr_positive`
- `her2_positive`
- `triple_negative`

These fields are used only for interoperability and distribution-shift checks. They do not make synthetic treatment-success labels equivalent to BreastDCEDL pCR labels or cBioPortal survival/recurrence labels.
...
- Public-distribution tuning can improve simulator realism on selected fields while also introducing new distortions. The current realism candidate is stored separately at `Data/external_bridge/realism_candidate/` and must remain an A/B candidate until it passes leakage, shortcut, calibration, counterfactual, and release-gate checks.
- The current public-distribution candidate passes as an A/B-only artifact, but the default dataset remains the current synthetic generator because no new exact-label clinical validation has been added.
