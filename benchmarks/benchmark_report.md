...
Generated at: 2026-07-30T10:22:10.377216+00:00
...
| Fixed-bank adversarial safety regression | supporting | acceptable | fresh | status=acceptable; total_cases=200; overall_attack_block_rate=0.955; hard_gate_passed=True; safe_answer_rate=1.000 | `Data/evals/safety/latest_adversarial_safety_regression.json` |
...
| Adversarial dev/holdout monitor | supporting | acceptable | fresh | status=acceptable; dev_pass_rate=0.953; holdout_pass_rate=0.961; holdout_n=51 | `Data/evals/safety/latest_adversarial_safety_regression_holdout.json` |
| Adversarial generalization eval | supporting | acceptable | fresh | status=acceptable; original_bank_pass_rate=0.955; heldout_pass_rate=0.969; paraphrase_pass_rate=0.927; safe_negative_control_pass_rate=1.000; category_gap_between_dev_and_holdout=0.125 | `Data/evals/safety/latest_adversarial_generalization_eval.json` |
...
| PoC service health snapshot | supporting | needs_attention | fresh | status=needs_attention; stale_artifact_count=121; failed_benchmark_count=0 | `Data/evals/ops/latest_service_health_snapshot.json` |
| Local synthetic backup and restore drill | supporting | strong_local_only | fresh | status=strong_local_only; passed=True; content_hash_match=True; strict_profile_validated=False; postgres_restore_tested=False; multi_instance_restore_tested=False; healthcare_production_ready=False | `Data/evals/ops/latest_deployment_recovery_drill.json` |
| Disposable Postgres/Redis migration and recovery smoke | supporting | blocked_environment | fresh | status=blocked_environment; completed=False; docker_available=False; healthcare_production_ready=False | `Data/evals/ops/latest_container_recovery_smoke.json` |
| Per-turn trace diagnostics coverage | supporting | needs_attention | fresh | status=needs_attention; rows_checked=100; rows_with_trace_diagnostics=50; rows_with_retrieval_confidence=0; sample_trace_schema_valid=True | `Data/evals/ops/latest_trace_diagnostics_coverage.json` |
| Cost and latency observability | supporting | strong | fresh | status=strong; request_count=500; latency_p50_ms=839.230; latency_p95_ms=3588.730; latency_sample_count=500; latency_percentile_credibility=stable_internal_sample; provider_reported_total_tokens=0; provider_usage_coverage_rate=0.000; estimated_pipeline_total_tokens=128535; estimated_total_cost_usd=0.000; cache_hit_rate=0.190; local_probe_stage_sample_count=708; normal_rag_probe_p95_ms=394.160; normal_rag_retrieval_p95_ms=152.500 | `Data/evals/ops/latest_cost_latency_report.json` |
...
| Route latency budget | supporting | acceptable | fresh | status=acceptable; route_count=7; needs_attention_count=0; highest_observed_p95_ms=394.350 | `Data/evals/ops/latest_route_latency_budget.json` |
...
| Behavior-adapter runtime preflight | supporting | blocked_runtime | fresh | status=blocked_runtime; model_trained=False; ready=False | `Data/evals/models/latest_finetune_runtime_preflight.json` |
...
| Fixed-baseline local retrieval runtime-cache evidence | supporting | improved_local_regression | fresh | status=improved_local_regression; retrieval_p95_delta_ms=-1653.820; sample_contract_met=True; production_ready=False | `Data/evals/ops/latest_retrieval_runtime_cache_eval.json` |
...
| One-pass internal author-contaminated adversarial holdout v7 | informational | needs_attention | fresh | status=needs_attention; pass_rate=0.676; unsafe_leakage_rate=0.355; over_refusal_rate=0.219 | `Data/evals/safety/latest_adversarial_holdout_v7_baseline.json` |
...
- medium: adversarial_holdout_v7_baseline (needs_attention) - Benchmark needs review before using it as supporting evidence.
