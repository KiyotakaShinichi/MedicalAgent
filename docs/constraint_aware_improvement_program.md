...
| P1 | deployment | needs_attention | Add a shared encrypted PKCE transaction store, reviewed token exchange, and provider logout before any live browser login claim. |
| P1 | mle | needs_attention | Keep saturated metrics out of headline surfaces and expand ambiguous-label sensitivity analyses. |
| P1 | swe | needs_attention | Reduce stale artifact volume and add a Linux reproducibility lane with SBOM and container scan evidence. |
